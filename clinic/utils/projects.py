...
from core.models import PatientProjectFieldValue
...
def add_project_fields(form_data):
    ...
    for item in range(len(field_ids)):
        field_value["projectField_id"] = PatientProjectsFields.objects.get(
            pk__exact=field_ids[item]
        )
        field_value["projectFieldValue"] = form_data[fields[item]]
        PatientProjectFieldValue.objects.create_project_field_value(
            field_value
        )
