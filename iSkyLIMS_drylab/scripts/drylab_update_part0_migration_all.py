...
def get_samples_info_for_migration(project_id):
    ...
    if Projects.objects.filter(pk__exact=project_id).exists():
        if SamplesInProject.objects.filter(project_id__exact=project_id).exists():
            ...
            return [
                [sample_obj.sample_name, sample_obj.pk] for sample_obj in sample_objs
            ]
    return None
...
def get_run_information(project_id):
    ...
    if len(sample_objs) > 0:
        path = RunningParameters.objects.get(
            runName_id=sample_objs[0].runProcess_id
        ).RunID
        return (
            sample_objs[0].runProcess_id.runName,
            sample_objs[0].runProcess_id.pk,
            path,
        )
    else:
...
def run():
        ...
        for line in fh.readlines():
            ...
            if Resolution.objects.filter(resolution_serviceID__pk__exact=s_id).exists():
                resolution_objs = Resolution.objects.filter(
                    resolution_serviceID__pk__exact=s_id
                )
    ...
    invalids = 0
    folder_files = os.path.join(
        settings.MEDIA_ROOT, USER_REQUESTED_SERVICE_FILE_DIRECTORY
    )
    ...
    with open("drylab_part3_service_files_to_migrate.csv", "r") as fh:
        for line in fh.readlines():
            ...
            if Service.objects.filter(pk__exact=s_id).exists():
                ...
                data = {
                    "file": split_line[1],
                    "file_name": os.path.basename(split_line[1]),
                }
                # import pdb; pdb.set_trace()
    ...
    with open("drylab_part4_project_used_in_services_to_migrate.csv", "r") as fh:
        for line in fh.readlines():
            ...
            data["run_name"], data["run_id"], data["sample_path"] = get_run_information(
                proj_id
            )
