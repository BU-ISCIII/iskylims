...
def run():
    ...
    if Service.objects.all().exists():
        ...
        ## dump platform
        with open('drylab_platform_to_migrate.csv', 'w') as fh:
            for service in services:
                ...
                if service.serviceSequencingPlatform != None : # for doing on develop
                    fh.write(str(str(service.pk) + ',' + service.serviceSequencingPlatform.platformName + '\n'))
        print('Successfully file creation for platform migration')
        # dump services states for apliying them to resolution
        with open('drylab_resolution_state_to_migrate.csv', 'w') as fh:
        ...
        with open('drylab_project_used_in_services_to_migrate.csv', 'w') as fh:
            for item in services:
