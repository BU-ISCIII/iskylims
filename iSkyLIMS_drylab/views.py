...
def request_sequencing_service(request):
	...
	if request.POST and request.FILES :
		if 'file' in request.FILES:
			...
			service_data_information = iSkyLIMS_drylab.utils.handling_request_services.prepare_form_data_request_service_sequencing(request)
	...
	if request.method == 'POST' and request.POST['subAction'] == 'createservice':
		...
		new_service = iSkyLIMS_drylab.utils.handling_request_services.create_new_save_sequencing_service_request(request)
		sample_stored = iSkyLIMS_drylab.utils.handling_request_services.stored_samples_for_sequencing_request_service(request.POST, new_service)
		if 'files' in request.POST:
			iSkyLIMS_drylab.utils.handling_request_services.add_files_to_service(request.POST.getlist('files'), new_service)
		...
		email_result = iSkyLIMS_drylab.utils.handling_request_services.send_service_creation_confirmation_email(email_data)
...
def counseling_request(request):
    ...
    if request.method == "POST" and request.POST["action"] == "createService":
        # check that at some services have been requested
        if len(request.POST.getlist('RequestedServices')) == 0 :
            service_data_information = iSkyLIMS_drylab.utils.handling_request_services.prepare_form_data_request_counseling_service()
            error_message = iSkyLIMS_drylab.drylab_config.ERROR_NO_SERVICES_ARE_SELECTED
            return render(request,'iSkyLIMS_drylab/requestCounselingService.html',{'service_data_information':service_data_information,
                           'error_message':error_message})

        new_service = iSkyLIMS_drylab.utils.handling_request_services.create_new_save_counseling_infrastructure_service_request(request)

        email_data = {}
        email_data['user_email'] = request.user.email
        email_data['user_name'] = request.user.username
        email_data['service_number'] = new_service.get_service_request_number()
        iSkyLIMS_drylab.utils.handling_request_services.send_service_creation_confirmation_email(email_data)
        confirmation_result = {}
        # confirmation_result['download_file'] = create_service_pdf_file(new_service.get_service_request_number(), request.build_absolute_uri())
...
def infrastructure_request(request):
    ...
    if request.method == "POST" and request.POST["action"] == "createService":
        # check that at some services have been requested
        if len(request.POST.getlist('RequestedServices')) == 0 :
            service_data_information = iSkyLIMS_drylab.utils.handling_request_services.prepare_form_data_request_infrastructure_service()
            error_message = iSkyLIMS_drylab.drylab_config.ERROR_NO_SERVICES_ARE_SELECTED
...
def add_samples_in_service(request):
    ...
    if request.method == 'POST' and request.POST['action'] == 'addeSamplesInService':
        ...
        service_data_information = iSkyLIMS_drylab.utils.handling_request_services.add_requested_samples_in_service(request)
...
def display_service(request, service_id):
    ...
    if iSkyLIMS_drylab.models.Service.objects.filter(pk=service_id).exists():
        service_manager = iSkyLIMS_drylab.utils.drylab_common_functions.is_service_manager(request)
        display_service_details = iSkyLIMS_drylab.utils.handling_request_services.get_service_information(service_id, service_manager)
...
def pending_services(request):
    ...
    pending_services_details = iSkyLIMS_drylab.utils.handling_request_services.get_pending_services_information()
    user_pending_services = iSkyLIMS_drylab.utils.handling_request_services.get_user_pending_services_information(request.user.username)
