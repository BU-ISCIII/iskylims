...
def graphic_multi_level_pie(
    heading, plottooltext, tooltext, theme, colors, source_data
):
    data_source = {}
    data_source["chart"] = {
        "caption": heading,
...
def column_graphic_per_time(
    heading, sub_caption, x_axis_name, y_axis_name, time_values, service_values
):
    data_source = {}
    ...
    for key in time_values:
        category_list.append(
            {"label": key, "stepSkipped": "false", "appliedSmartLabel": "true"}
        )
    ...
    for key, values in service_values.items():
        ...
        data_set_list.append(
            {"seriesname": key, "color": index_color[counter], "data": series_name_list}
        )
        counter += 1
