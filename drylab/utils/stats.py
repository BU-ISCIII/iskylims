...
def create_statistics_by_user(user_id, start_date=None, end_date=None):
    ...
    if len(service_objs) == 0:
        stats_info["ERROR"] = drylab.config.ERROR_NO_MATCHES_FOUND_FOR_YOUR_SERVICE_SEARCH
        return stats_info
        
    # create table of services
    stats_info["service_by_user"] = list(service_objs.values_list("pk", "service_request_number", "service_state__state_display"
        ).annotate(create_date=Func(
                F("service_created_date"),
                Value("%Y-%m-%d"),
                function="DATE_FORMAT",
                output_field=CharField(),
            )
        ).annotate(approved_date=Func(
                F("service_approved_date"),
                Value("%Y-%m-%d"),
                function="DATE_FORMAT",
                output_field=CharField(),
            )
        ).annotate(delivery_date=Func(
    ...
    serv_per_state = list(service_objs.values(sum_services=F("service_state__state_display")).annotate(value=Count("service_request_number")))
    g_data = core.utils.graphics.preparation_graphic_data(
        "Service states", "", "", "", "ocean", serv_per_state, "sum_services", "value"
    )
    stats_info[
        "user_services_graphic"
    ] = core.fusioncharts.fusioncharts.FusionCharts(
        "pie3d", "user_services_graph", "580", "300", "user_services_chart", "json", g_data
    ).render()
    ...
    serv_level_2_objs = service_objs.filter(service_available_service__in=avail_serv_level_2)
    services_level_2 = serv_level_2_objs.values(sum_avail_serv=F("service_available_service__avail_service_description")).annotate(value=Count("service_request_number"))
    g_data = core.utils.graphics.preparation_graphic_data(
          "Available services requested", "", "", "", "fint", services_level_2, "sum_avail_serv", "value"
    )
