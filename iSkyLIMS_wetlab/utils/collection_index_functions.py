...
def get_collection_index_information(collection_index_id):
    ...
    if CollectionIndexValues.objects.filter(
        ...
        I7_indexes.sort()
        collection_index_dict["I7_indexes"] = list(
            I7_indexes for I7_indexes, _ in itertools.groupby(I7_indexes)
        )
        if len(I5_indexes) > 0:
            I5_indexes.sort()
            collection_index_dict["I5_indexes"] = list(
                I5_indexes for I5_indexes, _ in itertools.groupby(I5_indexes)
            )
                ...
                if len(index_5):  # There are 2 index (I7 and I5)
                    index_values.append(
                        [
                            line_split[0],
                            line_split[1],
                            index_7[line_split[1]],
                            line_split[2],
                            index_5[line_split[2]],
                        ]
                    )
                else:
                    index_values.append(
                        [line_split[0], line_split[1], index_7[line_split[1]], "", ""]
                    )
    return index_values
...
def store_collection_settings(collection_settings, file_name):
    ...
    if "adapters" not in collection_settings:
