...
def get_researcher_statistics(researcher_name, start_date, end_date):
    ...
    if not User.objects.filter(username__icontains=researcher_name).exists():
        researcher_statistics[
            "ERROR"
        ] = wetlab_config.ERROR_NO_MATCHES_FOR_INPUT_CONDITIONS
    ...
    if len(user_objs) > 1:
        researcher_statistics[
            "ERROR"
        ] = wetlab_config.ERROR_MANY_USER_MATCHES_FOR_INPUT_CONDITIONS
        return researcher_statistics
    ...
    if start_date != "" and not check_valid_date_format(start_date):
        researcher_statistics[
            "ERROR"
        ] = wetlab_config.ERROR_INVALID_FORMAT_FOR_DATES
        return researcher_statistics
    ...
    if SamplesInProject.objects.filter(user_id=user_objs[0]).exists():
        ...
        ] = wetlab_config.RESEARCHER_SAMPLE_HEADING_STATISTICS
        ...
        q30_data_in_run = get_min_mean_and_max_values(
            q30_sample_value,
            runs_index_sample,
            wetlab_config.NUMBER_OF_VALUES_TO_FETCH_FROM_RESEARCHER,
        )
        ...
        if get_configuration_value("DESCRIPTION_IN_SAMPLE_SHEET_MUST_HAVE_USERNAME"):
            researcher_statistics[
                ...
                wetlab_config.ERROR_NOT_SAMPLES_FOR_USER_FOUND_BECAUSE_OF_CONFIGURATION_SETTINGS
            )
        else:
            researcher_statistics[
                "ERROR"
            ] = wetlab_config.ERROR_USER_DOES_NOT_HAVE_ANY_SAMPLE
