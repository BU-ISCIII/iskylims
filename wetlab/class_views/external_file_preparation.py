...
class ExternalFilePreparationView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request, self.template_name, {"search_data": self.get_search_data}
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.POST.get("action") == "externalFilePreparation":
            ...
            if "protocol" in request.POST:
                ...
                if core.models.Protocols.objects.filter(pk__exact=protocol_id).exists():
                    ...
                    if "DNA" in p_type or "RNA" in p_type:
                        ...
                        selecting_samples["samples"] = self.get_extraction_samples(
                            p_obj
                        )
                        if len(selecting_samples["samples"]) > 0:
                            selecting_samples["heading"] = (
                                core.core_config.HEADING_FOR_SELECTING_SAMPLE_EXTRACTION_ON_EXTRACTION
                            )
                            selecting_samples["heading_string"] = ",".join(
                                selecting_samples["heading"]
                            )
                            return render(
                                request,
            ...
            project_list = []
            if (
                "DNA" in request.POST["protocolType"]
                or "RNA" in request.POST["protocolType"]
            ):
                ...
                if len(extractions) > 0:
                    ...
                    for extraction in extractions:
                        sample_id = core.models.MoleculePreparation.objects.get(
                            pk__exact=extraction
                        ).sample
                        map_sample_extraction.setdefault(sample_id, []).append(
                            extraction
                        )
                    ...
                    for project, samples in sample_by_project.items():
                        ...
                        proj_fields = list(
                            core.models.SampleProjectsFields.objects.filter(
                                sample_projects_id=project,
                                sample_project_downloadable=True,
                            ).values_list("sample_project_field_name", flat=True)
                        )
                        data_for_download[project_name]["heading"] = (
                            ["sample name"]
                            + proj_fields
                            + ["extraction code"]
                            + down_param_list
                        ...
                        for sample in samples:
                            ...
                            s_proj_values = list(
                                core.models.SampleProjectsFieldsValue.objects.filter(
                                    sample_id=sample,
                                    sample_project_field_id__sample_project_field_name__in=proj_fields,
                            ...
                            for extract_id in map_sample_extraction[sample]:
                                extract_field_values = list(
                                    core.models.MoleculeParameterValue.objects.filter(
                                        molecule_id__pk=extract_id,
                                        molecule_parameter_id__in=down_param_obj,
                                    ).values_list("parameter_value", flat=True)
                                )
                                extract_code_id = (
                                    core.models.MoleculePreparation.objects.get(
                                        pk__exact=extract_id
                                    ).molecule_code_id
                                )
                                data_for_download[project_name].setdefault(
                                    "values", []
                                ).append(
                                    [s_name]
                                    + s_proj_values
    ...
    def get_extraction_samples(self, protocol_obj):
        ...
        if protocol_obj.extractions.filter(external_use=True).exists():
            ...
            return list(
                extract_objs.values_list(
                    "sample__sample_name", "molecule_code_id", "pk"
                )
            )
        return []
