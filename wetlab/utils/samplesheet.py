...
def validate_userid_in_user_iem_file (file_read, user_id_list):
        ...
        if data_section_found :
            ...
            if not description_index :
                try:
                    ...
                    users['ERROR'] = config.ERROR_SAMPLE_SHEET_DOES_NOT_HAVE_DESCRIPTION_FIELD
                    return users
            else:
                try:
    ...
    if len(invalid_names) > 0:
        ...
        invalid_names.insert(0,''.join(config.ERROR_SAMPLE_SHEET_FOLLOWING_USER_ARE_NOT_DEFINED))
...
def store_user_input_file (user_input_file):
    ...
    template_dir = os.path.join(settings.MEDIA_ROOT, config.LIBRARY_PREPARATION_SAMPLE_SHEET_DIRECTORY)
    ...
    file_name = str(config.LIBRARY_PREPARATION_SAMPLE_SHEET_DIRECTORY
