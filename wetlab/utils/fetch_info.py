...
def get_boxplot_comparation_runs(run_object):
    ...
    if (
        ...
        return
    run_lane_summary = wetlab.models.StatsLaneSummary.objects.filter(
    ...
    same_run_in_year = wetlab.utils.common.get_run_in_same_year_to_compare(run_object)
    ...
    if len(all_lane_summary) == 0:
        ...
        all_lane_summary = wetlab.models.StatsLaneSummary.objects.filter(
    ...
    q_30_run_normalized, q_30_all_normalized = wetlab.utils.common.normalized_data(
    ...
    data_source = wetlab.utils.stats_graphs.bloxplot_graphic(
...
def get_run_read_data(run_object, num_of_reads, number_of_lanes):
    ...
    for read_number in range(1, num_of_reads + 1):
        ...
        for lane_number in range(1, number_of_lanes + 1):
            read_lane_id = wetlab.models.StatsRunRead.objects.filter(
                runprocess_id__exact=run_object,
...
def get_run_summary_data(run_object, num_of_reads):
    ...
    for index in range(len(index_run_summary)):
        run_summary_id = wetlab.models.StatsRunSummary.objects.filter(
...
def get_information_run(run_object):
    ...
    p_list = wetlab.models.Projects.objects.filter(run_process=run_object)
    if p_list != "":
        # p_info = []
        # p_library_kit_list = []
    ...
    if wetlab.models.RunningParameters.objects.filter(run_name_id=run_object).exists():
        # Adding the Run Parameters information
    ...
    if run_state == "Completed":
        ...
        if len(wetlab.utils.common.get_run_in_same_year_to_compare(run_object)) > 1:
        ...
        for project_demultiplexion in p_list:
            if not wetlab.models.StatsLaneSummary.objects.filter(
                ...
                continue
            lanes_for_percent_graphic = wetlab.models.StatsLaneSummary.objects.filter(
                runprocess_id__exact=run_object, project_id=project_demultiplexion.id
            )
            percent_lane = []
        ...
        fl_summary_id = wetlab.models.StatsFlSummary.objects.filter(
        ...
        lane_data_display = []
        lane_summary_id = wetlab.models.StatsLaneSummary.objects.filter(
            runprocess_id__exact=run_object, project_id__isnull=True, default_all="all"
        )
        lane_list = [
            "Lane",
        ...
        default_fl_summary_id = wetlab.models.StatsFlSummary.objects.filter(
            runprocess_id__exact=run_object,
        ...
        default_lane_summary_id = wetlab.models.StatsLaneSummary.objects.filter(
            runprocess_id__exact=run_object,
            project_id__isnull=True,
            default_all="default",
        )
        ...
        for lane_un in range(number_of_lanes):
            ...
            unknow_bar_id = wetlab.models.RawTopUnknowBarcodes.objects.filter(
...
def get_information_project(project_id, request):
    ...
    if len(run_objs) == 1:
        ...
        project_info_dict["dates"] = list(
            zip(wetlab.config.HEADING_FOR_PROJECT_DATES, project_id.get_project_dates())
        )
...
def get_info_sample_in_run(sample_run_obj):
    ...
    theme = "fint"
    data_source = wetlab.utils.stats_graphs.column_graphic_one_column_highligthed(
...
def get_last_runs_by_sequencer():
    ...
    for sequencer in get_sequencer_installed_names():
        if wetlab.models.RunProcess.objects.filter(
            ...
            last_run_obj = wetlab.models.RunProcess.objects.filter(
                used_sequencer__sequencer_name__exact=sequencer
            ).last()
            last_runs.append(
                [
...
def get_sequencers_run_from_time_interval(sequencer, start_date, end_date):
    ...
    if core.models.SequencerInLab.objects.filter(sequencer_name__exact=sequencer).exists():
        ...
        if wetlab.models.RunProcess.objects.filter(
            ...
            run_objs_found = wetlab.models.RunProcess.objects.filter(
                used_sequencer=sequencer_obj, run_date__range=(start_date, end_date)
            )
...
def get_sequencer_installed_names():
    ...
    if core.models.SequencerInLab.objects.all().exists():
        sequencers = core.models.SequencerInLab.objects.all().order_by("platform_id")
...
def get_number_of_runs_per_sequencer():
    ...
    for sequencer in sequencer_list:
        if wetlab.models.RunProcess.objects.filter(
            ...
            number_of_runs[sequencer] = wetlab.models.RunProcess.objects.filter(
                used_sequencer__sequencer_name__exact=sequencer
            ).count()
