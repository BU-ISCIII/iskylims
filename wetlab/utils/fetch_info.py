...
from core.models import SequencerInLab
...
def get_boxplot_comparation_runs(run_object):
    ...
    if (
        not StatsLaneSummary.objects.filter(runprocess_id__exact=run_object)
        .exclude(default_all__isnull=False)
        .exists()
    ):
        # return empty information . No information for stats stored on database
        return
    run_lane_summary = StatsLaneSummary.objects.filter(
    ...
    all_lane_summary = (
        StatsLaneSummary.objects.filter(runprocess_id__in=same_runs_in_year_list)
        .exclude(default_all__isnull=False)
    ...
    if len(all_lane_summary) == 0:
        ...
        all_lane_summary = StatsLaneSummary.objects.filter(
    ...
    q_30_run_normalized, q_30_all_normalized = normalized_data(
        q_30_run_value_float, q_30_all_value_float
    )
    mean_run_normalized, mean_all_normalized = normalized_data(
        mean_run_value_float, mean_all_value_float
    )
    yield_mb_run_normalized, yield_mb_all_normalized = normalized_data(
        yield_mb_run_value_float, yield_mb_all_value_float
    )
    cluster_pf_run_normalized, cluster_pf_all_normalized = normalized_data(
    ...
    data = [
        [q30_run_str, mean_run_str, yield_mb_run_str, cluster_pf_run_str],
        [q_30_all_str, mean_all_str, yield_mb_all_str, cluster_pf_all_str],
    ]
    ...
    return FusionCharts(
        "boxandwhisker2d", "box1", "800", "400", "box_chart1", "json", data_source
    ).render()
...
def get_run_read_data(run_object, num_of_reads, number_of_lanes):
    ...
    for read_number in range(1, num_of_reads + 1):
        ...
        for lane_number in range(1, number_of_lanes + 1):
            read_lane_id = StatsRunRead.objects.filter(
                runprocess_id__exact=run_object,
                read__exact=read_number,
...
def get_run_summary_data(run_object, num_of_reads):
    ...
    for index in range(len(index_run_summary)):
        run_summary_id = StatsRunSummary.objects.filter(
            runprocess_id__exact=run_object, level__exact=index_run_summary[index]
        )
...
def get_running_parameters(run_object):
    ...
    rp_list = [
        ...
        "Chemistry",
        "Run Start Date",
        "Analysis Work Flow Type",
        "Run Management Type",
        "Planned Read1 Cycles",
        "Planned Read2 Cycles",
        "Planned Index1 Read Cycles",
        "Planned Index2 Read Cycles",
        "Application Version",
        "Num Tiles per Swatch",
        "Image Channel",
        "Flowcel",
        "Image Dimensions",
        "Flowcell Layout",
    ...
    for key, value in unknow_dict.items():
        ...
        if "+" in key:
            ...
            if CollectionIndexValues.objects.filter(
                ...
                libraries_using_base = CollectionIndexValues.objects.filter(
                    i_7_seq__exact=split_base[0]
                )
                index_temp = split_base[0]
                for library in libraries_using_base:
                    library_info.append(
                        [library.index_7, library.get_collection_index_name()]
                    )
            ...
            if CollectionIndexValues.objects.filter(
                ...
                for library in libraries_using_base:
                    library_info.append(
                        [library.index_5, library.get_collection_index_name()]
                    )
...
def get_information_for_incompleted_run():
    ...
    if RunProcess.objects.filter(state__run_state_name="Error").exists():
        run_information["error"] = []
        run_objs = RunProcess.objects.filter(state__run_state_name="Error").order_by(
            "run_name"
        )
        for run_obj in run_objs:
            data = []
    ...
    run_information["incompleted_graphic"] = FusionCharts(
...
def get_information_run(run_object):
    ...
    d_list = [
        "Run name",
        "State of the Run is",
        "Run was requested by",
        "Run was recorded on date",
        "Run date",
        "Run Completion Date",
    ]
    if run_state == "Completed":
        d_list += [
            "Bcl2Fastq finish Date",
            "Run Finish Date",
            "Disk space used for Images(in MB)",
            "Disk space used for Fasta Files(in MB)",
            "Disk space used for other Files(in MB)",
        ]
    ...
    p_list = Projects.objects.filter(run_process=run_object)
    if p_list != "":
        # p_info = []
        # p_library_kit_list = []
    ...
    if RunningParameters.objects.filter(run_name_id=run_object).exists():
        # Adding the Run Parameters information
    ...
    if StatsRunSummary.objects.filter(runprocess_id__exact=run_object).exists():
        ...
        info_dict["runSummaryTotal"], info_dict["runSummary"] = get_run_summary_data(
            run_object, num_of_reads
        )

        # prepare the data for Reads Binary summary stats
        info_dict["laneSummaryHeading"] = [
            "Lane",
            "Tiles",
            "Density (K/mm2)",
            "Cluster PF (%)",
        ...
        info_dict["reads"] = get_run_read_data(
            run_object, num_of_reads, number_of_lanes
        )
    ...
    if run_state == "Completed":
        ...
        for project_demultiplexion in p_list:
            if not StatsLaneSummary.objects.filter(
                runprocess_id__exact=run_object, project_id=project_demultiplexion.id
            ).exists():
                # continue with the stats for the next proyect in the run
                continue
            lanes_for_percent_graphic = StatsLaneSummary.objects.filter(
                runprocess_id__exact=run_object, project_id=project_demultiplexion.id
            )
            percent_lane = []
        ...
        default_lanes_for_percent_graphic = StatsLaneSummary.objects.filter(
        ...
        info_dict["run_project_comparation"] = FusionCharts(
            "column3d", "column1", "600", "400", "column_chart1", "json", data_source
        ).render()

        fl_data_display = []

        fl_summary_id = StatsFlSummary.objects.filter(
            runprocess_id__exact=run_object, project_id__isnull=True, default_all="all"
        )
        fl_list = [
            "Cluster (Raw)",
            "Cluster (PF)",
        ...
        lane_data_display = []
        lane_summary_id = StatsLaneSummary.objects.filter(
            runprocess_id__exact=run_object, project_id__isnull=True, default_all="all"
        )
        lane_list = [
            "Lane",
        ...
        default_fl_summary_id = StatsFlSummary.objects.filter(
            runprocess_id__exact=run_object,
        ...
        default_lane_summary_id = StatsLaneSummary.objects.filter(
            runprocess_id__exact=run_object,
            project_id__isnull=True,
            default_all="default",
        )
        ...
        for lane_un in range(number_of_lanes):
            ...
            unknow_bar_id = RawTopUnknowBarcodes.objects.filter(
                runprocess_id__exact=run_object, lane_number__exact=lane_number
            )
            #
...
def get_information_project(project_id, request):
    ...
    if len(run_objs) == 1:
        ...
        project_info_dict["dates"] = list(
            zip(HEADING_FOR_PROJECT_DATES, project_id.get_project_dates())
        )

        # prepare the data for Flowcell Summary
        if StatsFlSummary.objects.filter(
            project_id__exact=project_id, runprocess_id=run_obj
        ).exists():
            fl_summary_obj = StatsFlSummary.objects.filter(
                project_id__exact=project_id, runprocess_id=run_obj
            ).last()
            project_info_dict[
                "fl_summary_heading"
            ] = config.HEADING_SINGLE_PROJECT_FL_SUMMARY
            # fl_data_display.append(fl_list)
            project_info_dict["fl_summary_data"] = fl_summary_obj.get_fl_summary()

        # prepare the data for Lane Summary
        if StatsLaneSummary.objects.filter(
            project_id__exact=project_id, runprocess_id=run_obj
        ).exists():
            lane_summary_obj = StatsLaneSummary.objects.filter(
                project_id__exact=project_id, runprocess_id=run_obj
            )
            project_info_dict[
                "lane_summary_heading"
            ] = config.HEADING_SINGLE_PROJECT_STATS_LANE
            project_info_dict["lane_summary_data"] = []
            for lane_sum in lane_summary_obj:
                project_info_dict["lane_summary_data"].append(
                    lane_sum.get_lane_summary()
                )
        ...
        if SamplesInProject.objects.filter(
            ...
            project_info_dict[
                "sample_heading"
            ] = config.HEADING_SINGLE_PROJECT_SAMPLES
            project_info_dict["sample_data"] = []
            for sample_obj in sample_objs:
                project_info_dict["sample_data"].append(
                    [sample_obj.get_sample_id(), sample_obj.get_sample_information()]
                )
...
def get_info_sample_in_run(sample_run_obj):
    ...
    sample_info_dict["heading_samples_info"] = [
        "Sample",
        "Index used",
        "PF Cluster",
        "% of Project",
    ...
    theme = "fint"
    data_source = column_graphic_one_column_highligthed(
        heading_samples_in_project,
        sub_caption,
        x_axis_name,
        y_axis_name,
        theme,
...
def get_last_runs_by_sequencer():
    ...
    for sequencer in get_sequencer_installed_names():
        if RunProcess.objects.filter(
            used_sequencer__sequencer_name__exact=sequencer
        ).exists():
            last_run_obj = RunProcess.objects.filter(
                used_sequencer__sequencer_name__exact=sequencer
            ).last()
            last_runs.append(
                [
                    sequencer,
                    last_run_obj.get_run_platform(),
                    last_run_obj.get_run_id(),
                    last_run_obj.get_run_name(),
                ]
            )
    return last_runs
...
def get_sequencers_run_from_time_interval(sequencer, start_date, end_date):
    ...
    if SequencerInLab.objects.filter(sequencer_name__exact=sequencer).exists():
        ...
        if RunProcess.objects.filter(
            ...
            run_objs_found = RunProcess.objects.filter(
                used_sequencer=sequencer_obj, run_date__range=(start_date, end_date)
            )
...
def get_number_of_runs_per_sequencer():
    ...
    for sequencer in sequencer_list:
        if RunProcess.objects.filter(
            ...
            number_of_runs[sequencer] = RunProcess.objects.filter(
                used_sequencer__sequencer_name__exact=sequencer
            ).count()
...
def get_sequencer_names_from_platform(platform):
    ...
    """
    sequencer_list = []
    if SequencerInLab.objects.filter(platform_id__platform_name__exact=platform).exists():
        sequencers = SequencerInLab.objects.filter(
...
def get_stats_sequencer_data_from_selected_runs(
    ...
    for run_in_seq in runs_using_sequencer["completed_run_objs"]:
        sequencer_data["run_completed"].append(
            [
                run_in_seq.get_run_name(),
                run_in_seq.get_run_date(),
                run_in_seq.get_run_id(),
            ]
        )
    for run_in_seq in runs_using_sequencer["not_completed_run_objs"]:
        sequencer_data["not_run_completed"].append(
            [
                run_in_seq.get_run_name(),
                run_in_seq.get_run_date(),
                run_in_seq.get_run_id(),
    ...
    time_diference = []
    for run_in_seq in runs_using_sequencer["completed_run_objs"]:
        time_diference.append(
            (
                run_in_seq.get_run_finish_date_no_format().date()
                - run_in_seq.get_run_date_no_format()
            ).days
        )
    sequencer_data["mean_time_duration"] = format(
        statistics.mean(time_diference), ".2f"
    )
    ...
    sequencer_data["sequencer_runs_per_month_graph"] = FusionCharts(
        "column3d",
        "run_per_month_graph",
        "500",
        "400",
    ...
    if (
        RunProcess.objects.filter(state__run_state_name__exact="Completed")
        .exclude(used_sequencer__sequencer_name__exact=sequencer)
        .exists()
    ):
        runs_in_other_sequencers = RunProcess.objects.filter(
            state__run_state_name__exact="Completed"
        ).exclude(used_sequencer__sequencer_name__exact=sequencer)
        run_time_dict = {}
        ...
        sequencer_data["other_sequencers_runs_per_month_graph"] = FusionCharts(
            "column3d",
