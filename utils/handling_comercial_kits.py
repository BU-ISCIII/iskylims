...
def get_lot_commercial_kits(register_user_obj, protocol_obj):
    ...
    if UserLotCommercialKits.objects.filter(user = register_user_obj, basedCommercial__protocol_id = protocol_obj.type).exists():
        user_kits =UserLotCommercialKits.objects.filter(user = register_user_obj, basedCmomercial__protocol_id = protocol_obj.type)
